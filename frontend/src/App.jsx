...
const App = () => {
  // --- ESTADOS DE SESSÃO E TEMA ---
  const [currentUser, setCurrentUser] = useState(() => {
    const session = localStorage.getItem('nf:session');
    return session ? JSON.parse(session) : null;
  });
  
  const [authMode, setAuthMode] = useState(localStorage.getItem('nf:session') ? 'app' : 'login');
  const [isRegister, setIsRegister] = useState(false);
  const [authLoading, setAuthLoading] = useState(false);
  const [loginData, setLoginData] = useState({ nome: '', email: '', password: '' });
  const [dark, setDark] = useState(localStorage.getItem('nf:theme') === 'dark');
  ...
  const [userStats, setUserStats] = useState(() => {
    const saved = currentUser ? localStorage.getItem(`nf:stats:${currentUser.email}`) : null;
  ...
  }, [currentUser, dataSelecionada]);

  useEffect(() => {
    if (authMode === 'app') fetchMeals();
  }, [fetchMeals, authMode]);
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          todos.forEach(item => {
            ...
            if (!nomeChave) return;
            const formatado = { 
              c: Number(item.c) || 0, p: Number(item.p) || 0, 
              cho: Number(item.cho) || 0, g: Number(item.g) || 0, id: item._id 
            };
            mapaGeral[nomeChave] = formatado;
            if (item.userEmail !== "sistema@nutrifit.com") mapaUsuario[nomeChave] = formatado;
          });
          setDbFixa(mapaGeral);
          setMeusAlimentos(mapaUsuario);
        }
    ...
    if (authMode === 'app') carregarBaseAlimentos();
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        const sessao = { nome: data.nome, email: data.email, token: data.token };
        localStorage.setItem('nf:session', JSON.stringify(sessao));
        setCurrentUser(sessao);
        setAuthMode('app');
      } else {
  ...
  const handleAddAlimento = async () => {
    const nomeBusca = alInput.nome.toLowerCase().trim();
    const dbTotal = { ...dbFixa, ...meusAlimentos };
    const alimentoEncontrado = dbTotal[nomeBusca];
    const gramas = parseFloat(alInput.gramas);

    if (!alimentoEncontrado || !gramas) return alert("Verifique os dados!");
    ...
    const novaRef = {
      ...
      alimento: {
        nome: alInput.nome.toUpperCase(),
        gramas: gramas,
        cal: (alimentoEncontrado.c * gramas) / 100,
        p: (alimentoEncontrado.p * gramas) / 100,
        cho: (alimentoEncontrado.cho * gramas) / 100,
        g: (alimentoEncontrado.g * gramas) / 100
      }
    };
  ...
  const metas = useMemo(() => {
    ...
    const a = parseFloat(userStats.altura) || 170;
    const i = parseFloat(userStats.idade) || 25;
    let tmb = (10 * p) + (6.25 * a) - (5 * i) + (userStats.genero === 'masculino' ? 5 : -161);
    let kcal = tmb * 1.2;
    if (userStats.objetivo === 'perder') kcal -= 500;
    if (userStats.objetivo === 'ganhar') kcal += 500;
    return { kcal: Math.round(kcal), p: Math.round(p * 2), cho: Math.round((kcal * 0.4) / 4), g: Math.round(p * 0.8) };
  }, [userStats]);

  const { totais, incompleto } = useMemo(() => {
    const lista = historico[dataSelecionada] || [];
    const res = lista.reduce((acc, item) => ({
      kcal: acc.kcal + (Number(item.cal) || 0),
      p: acc.p + (Number(item.p) || 0),
      cho: acc.cho + (Number(item.cho) || 0),
      g: acc.g + (Number(item.g) || 0)
    }), { kcal: 0, p: 0, cho: 0, g: 0 });
    return { totais: res, incompleto: false };
  }, [historico, dataSelecionada]);

  const imcCalculado = useMemo(() => {
    const p = parseFloat(userStats.peso);
    const a = parseFloat(userStats.altura) / 100;
    return (p / (a * a)).toFixed(1);
  }, [userStats]);

  const statusIMC = useMemo(() => {
    const v = parseFloat(imcCalculado);
    if (v < 18.5) return { label: "Abaixo", color: "text-blue-500", bg: "bg-blue-500", pct: "25%", badge: "bg-blue-500/20" };
    if (v < 25) return { label: "Ideal", color: "text-emerald-500", bg: "bg-emerald-500", pct: "50%", badge: "bg-emerald-500/20" };
    if (v < 30) return { label: "Sobrepeso", color: "text-orange-500", bg: "bg-orange-500", pct: "75%", badge: "bg-orange-500/20" };
    return { label: "Obesidade", color: "text-red-500", bg: "bg-red-500", pct: "100%", badge: "bg-red-500/20" };
  }, [imcCalculado]);

  const gerarRelatorioSemanal = useCallback(() => {
    let sK = 0, sP = 0, sC = 0, sG = 0;
    Object.values(historico).forEach(dia => {
      dia.forEach(it => { sK += (it.cal || 0); sP += (it.p || 0); sC += (it.cho || 0); sG += (it.g || 0); });
    });
    setDadosSemana({ kcal: Math.round(sK/7), p: Math.round(sP/7), cho: Math.round(sC/7), g: Math.round(sG/7), insight: "Mantenha o foco!" });
    setShowRelatorio(true);
  }, [historico]);
  ...
  if (authMode !== 'app') {
    return (
      <div className={`min-h-screen flex items-center justify-center p-6 ${dark ? 'bg-slate-950 text-white' : 'bg-slate-50'}`}>
        <div className="w-full max-w-sm space-y-8">
          <div className="text-center">
            <Crown className="text-indigo-500 mx-auto mb-4" size={48} />
            <h1 className="text-4xl font-black italic uppercase tracking-tighter">NutriFit</h1>
          ...
          <div className="space-y-3">
            {isRegister && <input placeholder="NOME" className={inputClass} onChange={e => setLoginData({...loginData, nome: e.target.value.toUpperCase()})} />}
            <input placeholder="E-MAIL" className={inputClass} onChange={e => setLoginData({...loginData, email: e.target.value.toLowerCase()})} />
            <input type="password" placeholder="SENHA" className={inputClass} onChange={e => setLoginData({...loginData, password: e.target.value})} />
            <button onClick={handleAuth} className={btnPrimary}>{authLoading ? '...' : (isRegister ? 'CADASTRAR' : 'ENTRAR')}</button>
            <button onClick={() => setIsRegister(!isRegister)} className="w-full py-4 text-[10px] font-black uppercase opacity-40">
              {isRegister ? 'Já tem conta? Logue' : 'Criar conta'}
            </button>
          </div>
        </div>
      </div>
  ...
  // --- RENDER APP ---
  return (
    <div className={`min-h-screen pb-32 ${dark ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
      {/* MODAL RELATORIO */}
      {showRelatorio && dadosSemana && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-md bg-black/60">
          <div className={`relative w-full max-w-sm p-8 rounded-[3.5rem] border-2 shadow-2xl ${dark ? 'bg-slate-900 border-indigo-500/30' : 'bg-white border-slate-100'}`}>
            <button onClick={() => setShowRelatorio(false)} className="absolute top-6 right-6 opacity-30"><Plus size={24} className="rotate-45" /></button>
            <div className="text-center space-y-6">
              <h2 className="text-[10px] font-black uppercase tracking-widest opacity-40 italic">Média Semanal</h2>
              <div className="flex items-baseline justify-center gap-2"><span className="text-7xl font-black italic">{dadosSemana.kcal}</span><span className="text-lg opacity-20">KCAL</span></div>
              <div className="grid grid-cols-3 gap-4 py-4 border-y border-dashed border-slate-700/20">
                <div><p className="text-[8px] font-black text-orange-500">P</p><p className="font-black">{dadosSemana.p}g</p></div>
                <div><p className="text-[8px] font-black text-cyan-500">C</p><p className="font-black">{dadosSemana.cho}g</p></div>
                <div><p className="text-[8px] font-black text-amber-300">G</p><p className="font-black">{dadosSemana.g}g</p></div>
              </div>
              <button onClick={() => setShowRelatorio(false)} className={btnPrimary}>Fechar</button>
            </div>
          </div>
      ...
      <header className="max-w-md mx-auto px-6 pt-12 flex justify-between items-center">
        <div><p className="text-[10px] font-black opacity-40 uppercase">{formatarDataParaBR(dataSelecionada)}</p><h1 className="font-black text-2xl italic">{currentUser?.nome}</h1></div>
        <div className="flex gap-3">
          <button onClick={gerarRelatorioSemanal} className="p-4 rounded-2xl bg-amber-500/10 text-amber-600"><BarChart3 size={20}/></button>
          <button onClick={() => {setDark(!dark); localStorage.setItem('nf:theme', !dark ? 'dark' : 'light');}} className="p-4 rounded-2xl bg-indigo-600/10 text-indigo-600">{dark ? <Sun size={20}/> : <Moon size={20}/>}</button>
        </div>
      </header>
      ...
      <main className="max-w-md mx-auto px-6 mt-10">
        {step === 'resultado' ? (
          <div className="space-y-6">
            {isDiaAnterior && <div className="p-4 bg-indigo-500/10 text-indigo-500 rounded-2xl text-[10px] font-black uppercase text-center italic">Modo Visualização</div>}
            
            <div className={`p-10 rounded-[4rem] text-center border-4 ${totais.kcal > metas.kcal ? 'border-rose-500 bg-rose-500/5' : 'border-emerald-500 bg-emerald-500/5'}`}>
              <div className="flex items-baseline justify-center gap-2">
                <span className={`text-[80px] leading-none font-black italic ${totais.kcal > metas.kcal ? 'text-rose-500' : 'text-emerald-500'}`}>{Math.round(totais.kcal)}</span>
                <span className="text-xl font-black opacity-20">/ {metas.kcal}</span>
              </div>
              <div className="mt-8 space-y-4">
                {[
                  { label: 'Prot', atual: totais.p, meta: metas.p, cor: 'bg-orange-500' },
                  { label: 'Carb', atual: totais.cho, meta: metas.cho, cor: 'bg-cyan-500' },
                  { label: 'Gord', atual: totais.g, meta: metas.g, cor: 'bg-amber-300' }
                ].map(m => (
                  <div key={m.label} className="space-y-1">
                    <div className="flex justify-between text-[10px] font-black uppercase"><span>{m.label}</span><span className="opacity-40">{Math.round(m.atual)}g / {m.meta}g</span></div>
                    <div className="h-2 w-full rounded-full bg-slate-200 dark:bg-slate-800 overflow-hidden">
                      <div className={`h-full transition-all ${m.cor}`} style={{ width: `${Math.min((m.atual / m.meta) * 100, 100)}%` }} />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex gap-2">
                <div className="relative flex-[2]">
                  <input placeholder="ALIMENTO..." value={alInput.nome} onChange={e => {
                    const v = e.target.value; setAlInput({ ...alInput, nome: v });
                    const dbTotal = { ...dbFixa, ...meusAlimentos };
                    if (v.length > 1) setSugestoes(Object.keys(dbTotal).filter(n => n.includes(v.toLowerCase())));
                    else setSugestoes([]);
                  }} className={inputClass} />
                  {sugestoes.length > 0 && (
                    <div className="absolute z-50 w-full mt-2 rounded-2xl border-2 bg-white dark:bg-slate-900 shadow-xl overflow-hidden">
                      {sugestoes.slice(0, 5).map(s => <button key={s} onClick={() => { setAlInput({ ...alInput, nome: s.toUpperCase() }); setSugestoes([]); }} className="w-full p-4 text-left text-[10px] font-black uppercase hover:bg-indigo-600 hover:text-white">{s}</button>)}
                    </div>
                  )}
                </div>
                <input placeholder="G" type="number" value={alInput.gramas} onChange={e => setAlInput({ ...alInput, gramas: e.target.value })} className={`${inputClass} flex-1`} />
                <button onClick={handleAddAlimento} className="p-5 rounded-3xl bg-indigo-600 text-white"><Plus /></button>
              </div>
            )}
            ...
            <div className="space-y-4 pb-20">
              {(historico[dataSelecionada] || []).map(item => (
                <div key={item._id} className={`p-6 rounded-[2.5rem] border-2 flex justify-between items-center ${dark ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-sm'}`}>
                  <div>
                    <p className="font-black text-sm uppercase italic">{item.nome}</p>
                    <p className="text-[9px] opacity-40 font-bold uppercase">{item.gramas}G • {Math.round(item.cal)} KCAL</p>
                  </div>
                  {!isDiaAnterior && <button onClick={async () => {
                    await fetch(`${API_URL}/refeicoes/${item._id}`, { method: 'DELETE' });
                    fetchMeals();
                  }} className="text-rose-500/30 hover:text-rose-500 p-2"><Trash2 size={20} /></button>}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-6 pb-24">
            {profileFlow === 'menu' ? (
              <div className="space-y-4">
                <button onClick={() => setProfileFlow('calendario')} className={`w-full p-8 rounded-[2.5rem] border-2 flex justify-between items-center ${dark ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-xl'}`}>
                  <span className="flex items-center gap-4 text-indigo-500 font-black italic uppercase"><History /> Calendário</span>
                  <ChevronRight />
                </button>
                <button onClick={() => setProfileFlow('alimentos')} className={`w-full p-8 rounded-[2.5rem] border-2 flex justify-between items-center ${dark ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-xl'}`}>
                  <span className="flex items-center gap-4 text-amber-500 font-black italic uppercase"><UtensilsCrossed /> Meus Alimentos</span>
                  <ChevronRight />
                </button>
                <div className={`p-8 rounded-[2.5rem] border-2 ${dark ? 'bg-slate-900 border-slate-800' : 'bg-white'}`}>
                  <div className="flex justify-between items-end mb-4">
                    <div><p className="text-[10px] font-black uppercase opacity-30">Bio-Status</p><h2 className="text-4xl font-black italic">{imcCalculado} IMC</h2></div>
                    <span className={`text-[10px] font-black px-3 py-2 rounded-xl ${statusIMC.badge} ${statusIMC.color}`}>{statusIMC.label}</span>
                  </div>
                  <div className="h-3 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden"><div className={`h-full ${statusIMC.bg}`} style={{width: statusIMC.pct}}/></div>
                </div>
                <button onClick={() => setProfileFlow('setup')} className={btnPrimary}><Settings2 /> Bio-Medidas</button>
                <button onClick={() => {localStorage.clear(); window.location.reload();}} className="w-full p-4 font-black opacity-40 uppercase text-[10px] text-center underline">Sair da Conta</button>
              </div>
            ) : profileFlow === 'calendario' ? (
              <div className="space-y-4">
                <button onClick={() => setProfileFlow('menu')} className="flex items-center gap-2 font-black uppercase text-[10px] opacity-40"><ChevronLeft size={16}/> Voltar</button>
                <div className={`p-8 rounded-[3.5rem] border-2 ${dark ? 'bg-slate-900 border-slate-800' : 'bg-white shadow-xl'}`}>
                  <div className="flex justify-between items-center mb-8 font-black uppercase italic">
                    ...
                    <span className="text-xs">{viewDate.toLocaleString('pt-BR', { month: 'long', year: 'numeric' })}</span>
                  ...
                  <div className="grid grid-cols-7 gap-2">
                    {[...Array(new Date(viewDate.getFullYear(), viewDate.getMonth() + 1, 0).getDate())].map((_, i) => {
                      ...
                      const isSelecionado = dataLoopStr === dataSelecionada;
                      return (
                        <button key={i} onClick={() => {setDataSelecionada(dataLoopStr); setStep('resultado');}} className={`h-11 rounded-2xl text-[10px] font-black ${isSelecionado ? 'bg-indigo-600 text-white' : 'hover:bg-indigo-500/10'}`}>{diaNum}</button>
                      );
                    })}
                  </div>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className={`p-8 rounded-[3.5rem] border-2 space-y-6 ${dark ? 'bg-slate-900 border-slate-800' : 'bg-white'}`}>
                  <div className="grid grid-cols-2 gap-4">
                    <button onClick={() => setUserStats({...userStats, objetivo: 'perder'})} className={`p-6 rounded-3xl border-4 ${userStats.objetivo === 'perder' ? 'border-emerald-500 bg-emerald-500/10' : 'opacity-20'}`}><Flame size={28}/><span className="text-[10px] font-black">Cutting</span></button>
                    <button onClick={() => setUserStats({...userStats, objetivo: 'ganhar'})} className={`p-6 rounded-3xl border-4 ${userStats.objetivo === 'ganhar' ? 'border-orange-500 bg-orange-500/10' : 'opacity-20'}`}><Dumbbell size={28}/><span className="text-[10px] font-black">Bulking</span></button>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <input type="number" value={userStats.peso} onChange={e => setUserStats({...userStats, peso: e.target.value})} className={inputClass} placeholder="Peso" />
                    <input type="number" value={userStats.altura} onChange={e => setUserStats({...userStats, altura: e.target.value})} className={inputClass} placeholder="Altura" />
                  </div>
                  <button onClick={async () => {
                    await fetch(`${API_URL}/stats`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({email: currentUser?.email, ...userStats})});
                    localStorage.setItem(`nf:stats:${currentUser?.email}`, JSON.stringify(userStats));
                    setProfileFlow('menu');
                  }} className={btnPrimary}>Salvar Bio-Medidas</button>
                </div>
              </div>
            ) : null}
          </div>
      ...
      <nav className={`fixed bottom-10 left-1/2 -translate-x-1/2 w-80 p-2 flex justify-around backdrop-blur-2xl rounded-[3rem] border-2 ${dark ? 'bg-slate-900/80 border-slate-800' : 'bg-white/90 border-slate-200 shadow-2xl'}`}>
        <button onClick={() => {setStep('perfil'); setProfileFlow('menu');}} className={`p-5 rounded-[2rem] ${step === 'perfil' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}><User size={26}/></button>
        <button onClick={() => setStep('resultado')} className={`p-5 rounded-[2rem] ${step === 'resultado' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}><TrendingUp size={26}/></button>
      </nav>
    </div>
  );
};
